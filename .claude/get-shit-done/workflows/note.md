...
- **Global scope**: `/Users/zdohaim/Kilo_web/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/zdohaim/Kilo_web/.claude/notes/`)
...
2. Glob `/Users/zdohaim/Kilo_web/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/zdohaim/Kilo_web/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/zdohaim/Kilo_web/.claude/notes/` — works in any directory
